  ...
  public notificationAdd(i:number){
    this.notificationService.getAll().subscribe((result) => {
      ...
      if(chicken==0){
        ...
        if(!this.pleaseAdd(i) && !this.runningLow(i) && this.notification.type!=="fromFridge"){
  ...
  public addFromFridge(i:number){
    this.notificationService.getAll().subscribe((result) => {
      ...
      if(chicken==0){
        ...
        console.log("THIS IS THE ITEM" +this.chickenProduct[i].name)
        if(!this.pleaseAdd(i) && !this.runningLow(i)){
          for(var res of result){
            if(res.product===this.chickenProduct[i].name){
              console.log("ADD FROM FRIDGE "+this.chickenProduct[i].name)
              res.type="fromFridge";
              res.message="Add from Fridge ";
              //UPDATE
              this.notificationService.update(res).subscribe((result) => {
